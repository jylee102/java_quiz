...
public class BankApplication {
    ...
    private static void createAccount() {
        ...
        // accountArray 배열애 Account 객체를 저장하기
        for (int i = 0; i < accountArray.length; i++) {
            // accountArray의 디폴트값은 null이 들어있다.
            // ★ 계좌객체를 순서대로 저장하기 위해 null 체크
            if (accountArray[i] == null) {
                accountArray[i] = newAccount;
                System.out.println("결과: 계좌가 생성되었습니다.");
                // ★ 쓸데없는 반복은 피한다, newAccount 객체를 전부 저장하지 않는다.
                break;
            }
        }
    }
    ...
    private static void withdraw() {
        ...
        // '기존에 저장되어 있던 금액 + 새로 출금할 금액'을 빼서 잔고에 추가해준다.
        account.setBalance(account.getBalance() - money);
        System.out.println("결과: 출금이 완료되었습니다.");
    }

    // 5.계좌번호로 고객계좌 찾기 (Account 객체를 리턴해준다.)
    private static Account findAccount(String ano) {
        Account account = null; // return 하기 위해 for문 바깥에 변수 선언
